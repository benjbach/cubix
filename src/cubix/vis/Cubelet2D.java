...
//		
	     ...
	     gl.glTranslated(x_vc, y_vc, 0);
	     	...
	     	// DRAW FACES
	     	if(!vis.isNodeSMView() && !vis.isGraphSMView())
	     	{
	     		float[] c;
	     		...
	     		else if(vis.isSideView() || vis.isCubeView()) c = COLOR_SET.clone();
	     		else c = COLOR_WHITE.clone();
	     		gl.glColor3f(c[R], c[G], c[B]);
...
//				}
				...
				for(int i=0 ; i<slices.size() ; i++)
				{
					if(mode == 0)
						...
						s = mc.getHNodeSlice(i);
					else 
						s = mc.getVNodeSlice(i);
					...
					if(s instanceof TimeSlice){
						...
						r1 = i / (vis.getMatrixCube().getVNodeSlices().size());
						r2 = (i+1) / (vis.getMatrixCube().getVNodeSlices().size());
						v1 = corner[2];
						v2 = corner[1];
						v3 = corner[5];
						v4 = corner[4];
				...
				// DRAW WIRE
		     	...
		     	displayString(gl, Utils.add(pos, new float[]{width_vc/2, height_vc*1.3f}), "Time Slices", CubixVis.FONT_12, ALIGN_LEFT, FLOAT4_0.clone());
...
//		     	}	     	
	     	...
	     	{	
	     		...
	     		{
    			   num = Math.min(8,vis.getMatrixCube().getVNodeSlices().size());
	     			d_x = width_vc * .1f;
	     			d_y = height_vc/2 * .1f;
	         		gl.glTranslatef(num * d_x, -num * d_y, 0f);
	     			for(int t=0 ; t < num ; t++)
	     			{
     					s = cube.getVNodeSlice(num -t -1);
     					if(vis.isSelectedSlice(s))
     						gl.glColor3fv(COLOR_SLICE_SELECTED, 0);
     					else
     						gl.glColor4f(1f,1f,1f, 1);
...
//	     gl.glEnable(gl.GL_CULL_FACE);
	...
	protected void createDisplayLists(GL2 gl)
	{
		...
		gl.glBegin(GL2.GL_LINES);
			...
			gl.glColor4fv(vis.COLOR_AXIS_NODE, 0);
			gl.glVertex2fv(corner[1], 0); 
