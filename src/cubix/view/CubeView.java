...
public class CubeView extends CView {
	...
	public void init(CubixVis vis)
	{
		...
		Slice s;
		for(t=0 ; t < mc.getRowCount() ; t++)
		{
			s = mc.getVNodeSlice(t);
			pos = new float[]{CubixVis.CELL_UNIT * (t-(mc.getColumnCount()-1)/2f), 0f, 0f};
			positions.put(s, pos);
			angles.put(s, 0f);
			scales.put(s, 1f);
			labelTransparency.put(s, FLOAT4_0.clone());
			...
			s = mc.getHNodeSlice(t);
			pos = new float[]{0, CubixVis.CELL_UNIT * (t-(mc.getColumnCount()-1)/2f), 0f};
			positions.put(s, pos);
			angles.put(s, 0f);
			scales.put(s, 1f);
			labelTransparency.put(s, FLOAT4_0.clone());
		...
		float d = (float) (((mc.getRowCount() + mc.getTimeCount()) * vis.CELL_UNIT ) / (2 * Math.tan(viewAngle * Math.PI / 360)));
		...
		labelTransparency.get(mc.getVNodeSlice(0))[W] = 1;
		labelTransparency.get(mc.getVNodeSlice(0))[S] = 1;
	}
