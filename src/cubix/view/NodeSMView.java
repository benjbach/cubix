...
public class NodeSMView extends CView {
	...
	public void init(CubixVis vis) 
	{
		...
		float a = mc.getVNodeSlices().size() * (sliceHeight+yOffset) * (sliceWidth+zOffset); 
...
//		int  colCount = 11;
		Slice s;
		rowCount = 0;
		for(int t=0 ; t < mc.getVNodeSlices().size(); t++)
		{
		...
		labelTransparency.get(mc.getVNodeSlice(0))[N] = 1;
