...
public class TradeDataSet{
	...
	public static String TABLE = "trade";
	...
	private static int time_max = 2010;
		...
		try {
			...
			if(queryCountries.length > 0){
				for(String c : queryCountries){
					countryListOrigin += " OR source LIKE '" + c + "%'"; 
					countryListDest += " OR target LIKE '" + c + "%'"; 
				}
...
//			    	weight = res.getInt("export") + res.getInt("import");
//			    	weight = res.getInt("export");
//			    	if(weight <= 0) continue;
			    	...
			    	if(weight > 0){
			    		...
			    		numMin = Math.max(numMin, 0);
			    		CEdge e = new CEdge(utils.createEdgeID(v1.getID(), v2.getID(), t, true));
			    		e.setWeight(weight);
			    		tGraph.addEdge(e, v1, v2, t, true);
			    	}

			    	weight = res.getInt("import");
			    	if(weight > 0){
			    		numMax = Math.max(numMax, weight);
			    		numMin = Math.min(numMin, weight);
			    		numMin = Math.max(numMin, 0);
			    		CEdge e = new CEdge(utils.createEdgeID(v2.getID(), v1.getID(), t, true));
			    		e.setWeight(weight);
