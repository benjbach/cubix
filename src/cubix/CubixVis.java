...
public class CubixVis extends GLJPanel implements 	GLEventListener, 
		...
		public static final float[] COLOR_LABELS = new float[]{.6f,.6f,.6f, 1f};
		public static final float[] COLOR_LABELS_HIGHLIGHT = new float[]{0f,.0f,0f, 1f};
		public static final float[] COLOR_AXIS_NODE = new float[]{1,0,0,1f};
		...
		// LIGHT
		protected static final float LIGHT_VALUE_AMBIENT = .5f;
		protected static final float LIGHT_VALUE_DIFFUSE = 1f;
...
//		protected HashMap<Integer, float[]> valueColors = new HashMap<Integer, float[]>();
		...
		// LABELS
		protected float SCALE_LABELS = .15f;
...
//			edgeWeightColorScale.addContinuousColourRule(1/1, (int)(255), (int)(75), (int)(120));
			...
			{
				// NODE V-SLICES
				sv = matrixCube.getVNodeSlice(n);
				sliceScale.put(sv, 1f);
				sliceRotation.put(sv, 0f);
				slicePos.put(sv, new float[]{CELL_UNIT * (matrixCube.getColumnIndex(n)-(matrixCube.getColumnCount()-1)/2f), 0f, 0f}); 
				labelTransparency.put(sv, FLOAT4_0.clone());
				
				// NODE H-SLICES
				sh = matrixCube.getHNodeSlice(n);
				sliceScale.put(sh, 1f);
				sliceRotation.put(sh, 0f);
				slicePos.put(sh, new float[]{0f, CELL_UNIT * ((matrixCube.getColumnCount()-1)/2f - matrixCube.getRowIndex(n)), 0f}); 
			...
			for(Slice slice : matrixCube.getHNodeSlices()){
				slice.setRightLabelPos(currentView.getLabelPosR(slice));
				slice.setLeftLabelPos(currentView.getLabelPosL(slice));
			}
			for(Slice slice : matrixCube.getVNodeSlices()){
				slice.setRightLabelPos(currentView.getLabelPosR(slice));
				slice.setLeftLabelPos(currentView.getLabelPosL(slice));
			}
					...
					if(isFrontView() || isGraphSMView()){
						if(hoveredSlice != null){
							if(hoveredSlice instanceof VNodeSlice){
								col = matrixCube.getColumnIndex((CNode) hoveredSlice.getData());
							}else if(hoveredSlice instanceof HNodeSlice){
								row = matrixCube.getRowIndex((CNode) hoveredSlice.getData());
							}
...
//	      	displayStatistics(gl);   
	        ...
	        if(hoveredCell != null)
	        {
		    	...
		    	if(isFrontView()){
		    		...
		    		num /= 2;
		    		w /= 2; // Since edges are directed by nevertheless symmetric by definiton in this context.
		    	}else if(isSideView()){
		    		w = 0;
...
//    			}else 
	    ...
	    protected void _displayCell(GL2 gl, Cell c, float[] pos)
	    {
	    	...
	    	if(hoveredSlice != null)
	    	{
	    		if(hoveredSlice instanceof TimeSlice)
	    		{
	    			if(hoveredSlice.containsCell(c) || isSideView() || isGraphSMView() || isNodeSMView()){
	    				inTime = true;
	    			}else{
	    		...
	    		}else if(hoveredSlice instanceof HNodeSlice)
	    		{
	    			...
	    			if(hoveredSlice.containsCell(c) || isFrontView() || isGraphSMView() || isNodeSMView()){
	    				inVNode = true;
	    			}else{
				...
				if(selectedCell == null)
					...
					if(isFrontView() && queryMode == Query.SLICES){
							inHNode = false;
							inVNode = false;
					}else if(isSideView() && queryMode == Query.SLICES){
						inTime = false;
						inHNode = false;
					}
				}
	    	...
	    	if(translucency_label[N] > 0)
	    	{
	    		...
	    		for(col = 0 ; col < s.getColumnCount() ; col++)
	    		{
	    			if(s instanceof TimeSlice){
	    				s2 = matrixCube.getVNodeSlice(col);
	    			...
	    			}else if((isGraphSMView() || isNodeSMView()) && Utils.dist(camera.getLookAt(), camera.getPos()) > 65000){
	    				continue;
	    			}
	    				    			
	    			labelBounds.put(_display3DString(gl, posCorr, s2.getLabel(), thickness, wAlign, hAlign, translucency_label[N], labelColor, -alpha * ROTATION_FACTOR, scale),s2);
//	    			labelBounds.put(_display3DString(gl, posCorr, s2.getLabel(), thickness, wAlign, hAlign, translucency_label[N], labelColor, alpha, scale),s2);
	    	...
	    	if(translucency_label[E] > 0)
	    	{
	    		...
	    		for(row = 0 ; row < s.getRowCount() ; row++)
	    		{
	    			...
	    			if(s instanceof TimeSlice || s instanceof VNodeSlice){
	    				s2 = matrixCube.getHNodeSlice(row);
	    			}
	    			...
	    			pos = Utils.add(pos, offset);
	    			float[] posCorr = pos.clone();
	    			labelBounds.put(_display3DString(gl, posCorr, s2.getLabel(), thickness, wAlign, Align.CENTER, translucency_label[E], labelColor, 0, scale),s2);
	    	...
	    	if(translucency_label[S]> 0)
	    	{
	    		...
	    		for(col = 0 ; col < s.getColumnCount() ; col++)
	    		{
	    			...
	    			labelBounds.put(_display3DString(gl, posCorr, s2.getLabel(), thickness, wAlign, hAlign,  translucency_label[S], labelColor, -alpha * ROTATION_FACTOR, scale) ,s2);
//	    			labelBounds.put(_display3DString(gl, posCorr, s2.getLabel(), thickness, wAlign, hAlign,  translucency_label[S], labelColor, alpha, scale) ,s2);
	    	...
	    	if(translucency_label[W] > 0)
	    	{
	    		...
	    		for(row = 0 ; row < s.getRowCount() ; row++)
	    		{
	    			...
	    			pos = Utils.add(pos, offset);
	    			float[] posCorr = pos.clone();
	    			labelBounds.put(_display3DString(gl, posCorr, s2.getLabel(), thickness, wAlign, Align.CENTER,  translucency_label[W], labelColor, 0, scale),s2);
	    	    }
	    	}
	    }
...
//	    }
	    ...
	    protected float[] _display3DString(GL2 gl, float[] pos_3d, String string, int thickness, Align hAlign, Align vAlign, float transparency, float[] color, float alpha, float scale)
	    {
	    	GLUT glut = new GLUT();
...
//	    	drawSettingLine("", ++line, gl, glut);
	    	...
	    	drawSettingLine("GRAPH:", ++line, gl, glut);
	    	drawSettingLine("Nodes: " + tGraph.getVertexNumber(), ++line, gl, glut);
	    	drawSettingLine("Edges: " + tGraph.getEdgeNumber(), ++line, gl, glut);
...
//	    	gl.glClearColor(1f, 1f, 1f, 1f);
	    ...
	    protected void drawSettingLine(String string, int line, GL2 gl, GLUT glut)
	    {
...
//			Log.out(this, "dist " + dist);
	    ...
	    public Cell testForCellHit(Collection<Cell> cublets, float[] rayStart, float[] ray)
	    {
			...
			for(Cell c : cublets)
	    	{
	    		...
	    		if(((isGraphSMView() || isNodeSMView()) && dist < c.getScale())
	    		|| dist < c.getScale() / 2f)
	    		{
	    			if(closestCell == null)
	    			{
...
//			}
		...
		protected void initLight(GL2 gl)
	    {
	        ...
	        float[] lightColorAmbient = {LIGHT_VALUE_AMBIENT,LIGHT_VALUE_AMBIENT, LIGHT_VALUE_AMBIENT};
	        gl.glLightfv(GL2.GL_LIGHT0, GL2.GL_AMBIENT, lightColorAmbient, 0);
	        float[] lightColorDiffuse = {LIGHT_VALUE_DIFFUSE, LIGHT_VALUE_DIFFUSE, LIGHT_VALUE_DIFFUSE};
	    ...
	    {
	    	if((isCubeView() || isFrontView() || isSideView())
	    		&& (newView instanceof CubeView || newView instanceof FrontView || newView instanceof SideView)){
...
//			_updateCellVisibility();
		...
		public float[] getSlicePos(Slice m) { return slicePos.get(m).clone(); }
		...
		{ 
			...
			if(isVSliceMode())
			{
			    renderedSlices.addAll(matrixCube.getVNodeSlices());
			    Collections.reverse(renderedSlices);
			}else if(isHSliceMode()){
			    renderedSlices.addAll(matrixCube.getHNodeSlices());
			}else{
...
//			} catch (IOException e) { e.printStackTrace(); }
		...
		public boolean isFrontView(){ return currentView == vm.getView(vm.VIEW_FRONT); }
		public boolean isSideView(){ return currentView == vm.getView(vm.VIEW_SIDE); }
		public boolean isCubeView(){ return currentView == vm.getView(vm.VIEW_CUBE); }
		public boolean isGraphSMView(){ return currentView == vm.getView(vm.VIEW_GRAPH_SM); }
		public boolean isNodeSMView(){ return currentView == vm.getView(vm.VIEW_NODE_SM); }
