...
//		Log.out(this, "Start transition.");
	...
	public Transition getTransition(CView currentView, CView targetView)
	{
		...
		trans.put(SideView.class, this.TRANS_CUBE_SIDE);
		...
		trans.put(SideView.class, this.TRANS_FRONT_SIDE);

		// SIDE
		trans = new HashMap<Class, Integer>();
		transitions.put(SideView.class, trans);
...
//		Log.out(this, "durFac " + durFac);
		...
		switch(tNum)
		{
			...
			case TRANS_CUBE_SIDE	: t = this.getCube2SideTransition( 600 * durFac);break;
...
//	}
	...
	public Transition getNodeSM2CubeTransition(float duration)
	{
		...
		for(int t=0 ; t < cube.getVNodeSlices().size() ; t++)
		{
...
//	}
	...
	protected Transition getFront2CubeTransition(float f)
	{
		...
		Slice s;
		for(int n=0 ; n < cube.getVNodeSlices().size() ; n++){
			s = cube.getVNodeSlice(n);
			a = new SliceTranslationAnimation(s, targetView.getSlicePosition(s));
	...
	protected Transition getCube2SideTransition(float duration)
	{
		Camera c = vis.getCamera();
		CView targetView = vm.getView(vm.VIEW_SIDE);
		Transition trans = new Transition(vis, SliceMode.VNODE);	
		Animation a;
