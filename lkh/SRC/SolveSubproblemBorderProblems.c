...
void SolveSubproblemBorderProblems(int Subproblems)
{
    ...
    do {
        ...
        N->Suc->Pred = N;
        if (!Fixed(N, N->Suc))
            GlobalBestCost += Distance(N, N->Suc);
...
static void MarkBorderPoints(int CurrentSubproblem)
{
    ...
    do {
        if (N->Subproblem == CurrentSubproblem ||
            ...
            N->Rank = INT_MAX;
            for (i = CoordType == THREED_COORDS ? 2 : 1; i >= 0; i--) {
                dMin = fabs(Coord(N, i) - Min[i]) + 0.5;
                dMax = fabs(Coord(N, i) - Max[i]) + 0.5;
            ...
            if (CoordType == TWOD_COORDS) {
                ...
                axis = 0;
            if (axis != -1) {
                dMin = fabs(Coord(N, axis) - Min[axis]) + 0.5;
                dMax = fabs(Coord(N, axis) - Max[axis]) + 0.5;
 ...
 * smallest element. Rank[i] contains the value of A[i].
 */
