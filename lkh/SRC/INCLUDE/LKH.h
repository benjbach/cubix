...
enum InitialTourAlgorithms { BORUVKA, GREEDY, NEAREST_NEIGHBOR,
    QUICK_BORUVKA,
    SIERPINSKI, WALK
};
...
typedef struct Node {
    ...
    *FixedTo2;  /* A maximum of two fixed edges can be incident
    ...
    struct Node *OptimumSuc;    /* Successor in the INPUT_TOUR file */
...
/* The Segment strucure is used to represent the segments in the two-level 
   representation of tours */

typedef struct Segment {
    short Reversed;     /* Reversal bit */
...
typedef struct SSegment {
    ...
    int Rank;   /* The ordinal number of the segment in the
                   list */
...
int CandidateSetSymmetric, CandidateSetType,
    ...
    PatchingCExtended, PatchingCRestricted, ProblemType,
...
void SolveSierpinskiSubproblems(void);
