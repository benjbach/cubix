 ...
 *   
 * The function is called from BestKOptMove and BacktrackKOptMove.   
 */
...
static int Patchwork = 0, RecLevel = 0;
...
static GainType PatchCyclesRec(int k, int m, int M, GainType G0)
{
    ...
    for (Ns2 = s2->CandidateSet; (s3 = Ns2->To); Ns2++) {
        ...
        for (X4 = 1; X4 <= 2; X4++) {
            ...
            if (FixedOrCommon(s3, s4) || Deleted(s3, s4, k))
                continue;
            t[2 * (k + m)] = s4;
            G2 = G1 + C(s3, s4);
            if (M > 2) {
                ...
                memcpy(cycle, cycleSaved, (1 + 2 * k) * sizeof(int));
    ...
    if (M == 2 && !PatchingCRestricted) {
        ...
        for (Ns2 = s2->CandidateSet; (s3 = Ns2->To); Ns2++) {
            ...
            for (X4 = 1; X4 <= 2; X4++) {
                s4 = X4 == 1 ? s3->Pred : s3->Suc;
                if (FixedOrCommon(s3, s4) || Deleted(s3, s4, k + m - 1))
                    continue;
                t[2 * (k + m)] = s4;
                G2 = G1 + C(s3, s4);
                Breadth4 = 0;
                ...
                for (Ns4 = s4->CandidateSet; (s5 = Ns4->To); Ns4++) {
                    ...
                    for (X6 = 1; X6 <= 2; X6++) {
                        ...
                        if ((!c || G4 - c(s6, s1) > 0) &&
                            ...
                            memcpy(p, pSaved, (1 + 2 * k) * sizeof(int));
                            for (i = 1; i <= 2 * k; i++)
                                q[p[i]] = i;
                        }
                    }
    ...
    if (S4) {
        ...
        if (!pSaved) {
            assert(pSaved = (int *) malloc((1 + 2 * k) * sizeof(int)));
