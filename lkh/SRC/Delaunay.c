...
static float nextafterf(const float x, const float y);
#endif
    ...
    for (i = 0; i < nsites; i++) {
        for (j = i + 1; j < nsites; j++)
            if ((float) sites[j]->X != (float) sites[i]->X ||
                ...
                break;
        for (j--; i < j; i++)
            sites[i + 1]->Y = nextafterf(sites[i]->Y, FLT_MAX);
    }
...
static int compare(const void *s1, const void *s2)
{
    float x1 = (*((site_struct **) s1))->X;
    float y1 = (*((site_struct **) s1))->Y;
    float x2 = (*((site_struct **) s2))->X;
    float y2 = (*((site_struct **) s2))->Y;
