...
static void CheckSpecificationPart()
{
    ...
    if (CandidateSetType == NEAREST_NEIGHBOR && !TwoDWeightType()
    ...
    if (ExtraCandidateSetType == NEAREST_NEIGHBOR && !TwoDWeightType()
...
static void Read_NODE_COORD_SECTION()
{
    ...
    for (i = 1; i <= Dimension; i++) {
        ...
        if (Name && (!strcmp(Name, "tsp225") || !strcmp(Name, "d657"))) {
            N->X = (float) N->X;
            N->Y = (float) N->Y;
        }
    }
...
/*
   The ReadTour function reads a tour from a file, if requested in the parameter 
   file (by using the INPUT_TOUR keyword). The tour is used to limit the search 
   (the last edge to be excluded in a non-gainful move must not belong to the
   tour). 
