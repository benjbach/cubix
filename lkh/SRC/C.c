...
{
    int dx = ceil(0.31622 * fabs(Na->X - Nb->X)),
        dy = ceil(0.31622 * fabs(Na->Y - Nb->Y));
    return (dx > dy ? dx : dy) * Precision + Na->Pi + Nb->Pi;
}

int c_CEIL_2D(Node * Na, Node * Nb)
{
    int dx = ceil(fabs(Na->X - Nb->X)), dy = ceil(fabs(Na->Y - Nb->Y));
    return (dx > dy ? dx : dy) * Precision + Na->Pi + Nb->Pi;
}

int c_CEIL_3D(Node * Na, Node * Nb)
{
    int dx = ceil(fabs(Na->X - Nb->X)),
...
{
    int dx = fabs(Na->X - Nb->X) + 0.5, dy = fabs(Na->Y - Nb->Y) + 0.5;
    return (dx > dy ? dx : dy) * Precision + Na->Pi + Nb->Pi;
}

int c_EUC_3D(Node * Na, Node * Nb)
{
    int dx = fabs(Na->X - Nb->X) + 0.5,
...
int c_GEO(Node * Na, Node * Nb)
{
    ...
    int dx =
        RRR * PI / 180.0 * fabs(da - db + 5.0 * (ma - mb) / 3.0) + 1.0;
    return dx * Precision + Na->Pi + Nb->Pi;
}
...
{
    int dx = M_RRR * M_PI / 180.0 * fabs(Na->X - Nb->X) + 1.0;
    return dx * Precision + Na->Pi + Nb->Pi;
}
...
int c_GEO_MEEUS(Node * Na, Node * Nb)
{
    ...
    double ma = Na->X - da, mb = Nb->X - db;
    int dx =
        RRR * M_PI / 180.0 * fabs(da - db + 5.0 * (ma - mb) / 3.0) * f +
        0.5;
    return dx * Precision + Na->Pi + Nb->Pi;
}
...
{
    int dx = M_RRR * M_PI / 180.0 * fabs(Na->X - Nb->X) * f + 0.5;
    return dx * Precision + Na->Pi + Nb->Pi;
}
